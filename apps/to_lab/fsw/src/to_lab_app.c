...
int32 TO_LAB_init(void)
{
    ...
    */
    ...
    CFE_EVS_SendEvent(TO_INIT_INF_EID, CFE_EVS_EventType_INFORMATION, "TO Lab Initialized to port %d.%s, Awaiting enable command.",
                      cfgTLM_PORT, TO_LAB_VERSION_STRING);
    ...
    do
    {
        ...
        if ((CFE_SB_status == CFE_SUCCESS) && (TO_LAB_Global.suppress_sendto == false))
        {
            ...
            if (TO_LAB_Global.downlink_on == true)
            {
                ...
                // CFE_EVS_SendEvent(TO_REMOVEALLPKTS_INF_EID, CFE_EVS_EventType_INFORMATION,
                //       "Sending message to %d", d_addr.AddrData.Buffer[0]);
                //       CFE_EVS_SendEvent(TO_REMOVEALLPKTS_INF_EID, CFE_EVS_EventType_INFORMATION,
                //       "Sending message to %d", d_addr.AddrData.Buffer[1]);
