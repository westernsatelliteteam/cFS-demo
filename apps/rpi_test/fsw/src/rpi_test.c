...
** Include Files:
*/
#include "rpi_test_events.h"
#include "rpi_test_version.h"
#include "rpi_test.h"
...
void RPI_TEST_Main(void)
{
    ...
    */
    CFE_ES_PerfLogEntry(RPI_TEST_PERF_ID);
    ...
    */
    status = RPI_TEST_Init();
    if (status != CFE_SUCCESS)
    {
        RPI_TEST_Data.RunStatus = CFE_ES_RunStatus_APP_ERROR;
    }

    /*
    ** SAMPLE Runloop
    */
    while (CFE_ES_RunLoop(&RPI_TEST_Data.RunStatus) == true)
    {
        /*
        ** Performance Log Exit Stamp
        */
        CFE_ES_PerfLogExit(RPI_TEST_PERF_ID);

        /* Pend on receipt of command packet */
        status = CFE_SB_ReceiveBuffer(&SBBufPtr, RPI_TEST_Data.CommandPipe, CFE_SB_PEND_FOREVER);
        ...
        */
        ...
        if (status == CFE_SUCCESS)
        {
            // RPI_TEST_ProcessCommandPacket(SBBufPtr);
        }
        else
        {
            CFE_EVS_SendEvent(RPI_TEST_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
                              "RPI TEST: SB Pipe Read Error, App Will Exit");
    ...
    */
    ...
    CFE_ES_ExitApp(RPI_TEST_Data.RunStatus);
...
int32 RPI_TEST_Init(void)
{
    ...
    RPI_TEST_Data.RunStatus = CFE_ES_RunStatus_APP_RUN;

    /*
    ** Initialize app command execution counters
    */
    RPI_TEST_Data.CmdCounter = 0;
    RPI_TEST_Data.ErrCounter = 0;

    /*
    ** Initialize app configuration data
    */
    RPI_TEST_Data.PipeDepth = RPI_TEST_PIPE_DEPTH;

    strncpy(RPI_TEST_Data.PipeName, "RPI_TEST_CMD_PIPE", sizeof(RPI_TEST_Data.PipeName));
    RPI_TEST_Data.PipeName[sizeof(RPI_TEST_Data.PipeName) - 1] = 0;
    ...
    ** Register the events
    */
    status = CFE_EVS_Register(RPI_TEST_Data.EventFilters, RPI_TEST_EVENT_COUNTS, CFE_EVS_EventFilter_BINARY);
    if (status != CFE_SUCCESS)
    {
        CFE_ES_WriteToSysLog("Rpi Test: Error Registering Events, RC = 0x%08lX\n", (unsigned long)status);
        return (status);
    }
    ...
    */
    // CFE_MSG_Init(&RPI_TEST_Data.HkTlm.TlmHeader.Msg, RPI_TEST_HK_TLM_MID, sizeof(RPI_TEST_Data.HkTlm));

    /*
    ** Create Software Bus message pipe.
    */
    status = CFE_SB_CreatePipe(&RPI_TEST_Data.CommandPipe, RPI_TEST_Data.PipeDepth, RPI_TEST_Data.PipeName);
    if (status != CFE_SUCCESS)
    {
        CFE_ES_WriteToSysLog("Rpi Test: Error creating pipe, RC = 0x%08lX\n", (unsigned long)status);
        return (status);
    }
    ...
    */
    // status = CFE_SB_Subscribe(RPI_TEST_SEND_HK_MID, RPI_TEST_Data.CommandPipe);
    // if (status != CFE_SUCCESS)
    // {
    //     CFE_ES_WriteToSysLog("Rpi Test: Error Subscribing to HK request, RC = 0x%08lX\n", (unsigned long)status);
    //     return (status);
    // }
    ...
    */
    status = CFE_SB_Subscribe(RPI_TEST_CMD_MID, RPI_TEST_Data.CommandPipe);
    if (status != CFE_SUCCESS)
    {
    ...
    CFE_EVS_SendEvent(RPI_TEST_STARTUP_INF_EID, CFE_EVS_EventType_INFORMATION, "Rpi Test Initialized.%s",
                      RPI_TEST_VERSION_STRING);
